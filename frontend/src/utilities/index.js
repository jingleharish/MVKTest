...
export function formatSpeed(duration, points) {
  if (0 === points) {
    ...
    return [
      `${sec}`,
      ".",
      TRIPLE_ZERO.substring(0, TRIPLE_ZERO.length - ms.length),
      ms,
      " seconds",
    ].join("");
  }
...
export function isValidRoomCode(roomCodeRaw) {
  ...
  return (
    ROOM_CODE_MIN <= roomCode && roomCode <= ROOM_CODE_MAX && !isNaN(roomCode)
  );
}
...
export function onHostStartGame(title) {
  document.title = title || "MaverickQuizHub";
  beforeUnloadWarning = [
...
export function onExitGame() {
  document.title = "MaverickQuizHub";
